...
def _infer_from_artifact(artifact: Dict[str, Any]) -> Tuple[str, str, str, List[str]]:
    ...
    # Prefer KB triage CVE candidates if available (artifact self-contained)
    triage = analysis.get("triage") if isinstance(analysis, dict) else None
    triage_cves: List[str] = []
        ...
        if isinstance(cc, list):
            for item in cc:
                if isinstance(item, dict):
                    ...
                    if cid:
                        triage_cves.append(str(cid))

    # Prepend triage CVEs to planning_keywords (dedup, preserve order)
    if triage_cves:
        seen = set()
        merged: List[str] = []
        for x in triage_cves + planning_keywords:
            sx = str(x).strip()
