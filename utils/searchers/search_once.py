...
def compose(target_dir: str, keyword: str, s1=GoogleSearcher(), s2=GithubSearcher(), s3=ExploitDBSearcher(), loose_mode: bool = False):
    ...
    if not os.path.exists(target_dir):
        ...
        # Do not short-circuit on cache hits.
        # In practice, prior runs often create the directory but leave it empty
        # (e.g., transient network errors, missing tokens, rate limits). If we
        # return early here, the PlanningAgent will repeatedly produce 0 results
        # and 0 scoring entries.
        print(f"{target_dir} exists! Reusing directory and continuing search.")
